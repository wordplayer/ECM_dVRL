...
class ECMEnv(gym.GoalEnv):
    def __init__(self, psm_num, n_actions, n_states, n_goals, n_substeps, camera_enabled, scene_path):
        ...
        self._env_setup()
        self.done = False
        self.desired_goal = np.array([216., 270.])
        self.bounds = [[-75, 45], [-45, 65], [0, 0.235], [-90, 90]]
        self.init_angles = self.ecm.getJointAngles()
    ...
    def _set_action(self, action):
        ...
        self._step_callback(obs, action) 
    ...
    def _is_success(self):
        ...
        return (np.linalg.norm(achieved - goal) < 0.1)
    ...
    def _step_callback(self, obs, action):
        ...
        """
        new_pos = obs + action
        if not (new_pos[0] >= np.radians(self.bounds[0][0])):
            new_pos[0] = self.bounds[0][0]
        elif not (new_pos[0] <= np.radians(self.bounds[0][1])):
            new_pos[0] = self.bounds[0][1]
            
        if not (new_pos[1] >= np.radians(self.bounds[1][0])):
            new_pos[1] = self.bounds[1][0]
        elif not (new_pos[1] <= np.radians(self.bounds[1][1])):
            new_pos[1] = self.bounds[1][1]
            
        if not (new_pos[2] >= self.bounds[2][0]):
            new_pos[2] = self.bounds[2][0]
        elif not (new_pos[2] <= self.bounds[2][1]):
            new_pos[2] = self.bounds[2][1]
            
        if not (new_pos[3] >= np.radians(self.bounds[3][0])):
            new_pos[3] = self.bounds[3][0]
        elif not (new_pos[3] <= np.radians(self.bounds[3][1])):
            new_pos[3] = self.bounds[3][1]
